...
AllSh=sum((AllYh-MultiDataInput(1).w).^2,2); 
for id=1:1:100
    id
    DataInput=MultiDataInput(id);
    Dl=DataInput.Dl;
    Dh=DataInput.Dh;
    Yl=(DataInput.Yl)';    
    Yh=(DataInput.Yh)';
    ...
    [U10,S10,~]=svd(Ylc);
    S10=diag(S10).^2;
    PossibleFractions=cumsum(S10)/sum(S10);
    ...
    Uh=Uh0(:,1:ph);
    yhproj=@(x) computeoutput(x,Ul,ybarl,Uh,Deltabarh,Case);
    
    ApproxShFun=@(x) sum((yhproj(x)-w).^2);
    TrueSh=@(x) sum((Simulator(x,2,Case)'-w).^2);
    xhatstarMLEnd=T_SVD_AGP{id}.xhatstarMLs(end,:);
    xhatstarMLsEnd(id,:)=xhatstarMLEnd;
    TrueShxhatstarMLsEnd(id,1)=T_SVD_AGP{id}.ShxhatstarMLs(end,:);    
    ApproxShxhatstarMLsEnd(id,1)=ApproxShFun(xhatstarMLEnd);   
   
    yhproj_ylyh=@(yl,yh) computeoutput_ylyh(yl,yh,Ul,ybarl,Uh,Deltabarh);
    ApproxShFun_ylyh=@(yl,yh) sum((yhproj_ylyh(yl,yh)-w).^2);
    parfor jd=1:NGP
        ApproxShsGrid(jd,id)=ApproxShFun_ylyh(AllYl(jd,:),AllYh(jd,:));
    end
    ...
    xhatstarMLs=T_SVD_AGP{id}.xhatstarMLs((nl+nh):2:end,:);   
    TrueShxhatstarMLs=T_SVD_AGP{id}.ShxhatstarMLs((nl+nh):2:end,:)';       
...
save DataforAppendixJ_SVDAGP.mat

%%
function ProjectedHFOutput=computeoutput(x,Ul,ybarl,Uh,Deltabarh,Case)
    yh=Simulator(x,2,Case);
    yl=Simulator(x,1,Case);
    ProjectedHFOutput=Ul*Ul'*(yl'-ybarl)+ybarl+Uh*Uh'*((yh'-yl')-Deltabarh)+Deltabarh;
end

function ProjectedHFOutput=computeoutput_ylyh(yl,yh,Ul,ybarl,Uh,Deltabarh)
    ProjectedHFOutput=Ul*Ul'*(yl'-ybarl)+ybarl+Uh*Uh'*((yh'-yl')-Deltabarh)+Deltabarh;
end